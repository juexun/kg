<template>
  ...
  <div id="graphcontainer" class="graphcontainer" style="background: lightblue"></div>
</template>
  ...
  methods: {
    ...
    fin() {
      ...
      $("body").bind("mousedown", function(event) {
        if (
          !(
            event.target.id === "link_menubar" ||
            $(event.target).parents("#link_menubar").length > 0
          )
        ) {
          $("#link_menubar").hide();
        }
